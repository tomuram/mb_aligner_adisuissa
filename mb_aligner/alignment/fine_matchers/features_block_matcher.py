...
#             setattr(threadLocal, fine_matcher_key, fine_matcher)
        ...
        logger.report_event("Features-Matching+PMCC layers: {} -> {} valid matches: {}, invalid_matches: {} {}".format(sec1.canonical_section_name, sec2.canonical_section_name, len(valid_matches1[0]), len(invalid_matches1[0]), FeaturesBlockMatcherDispatcher.sum_invalid_matches(invalid_matches1)), log_level=logging.INFO)

        logger.report_event("Features-Matching+PMCC layers: {} <- {}".format(sec1.canonical_section_name, sec2.canonical_section_name), log_level=logging.INFO)
        valid_matches2, invalid_matches2 = fine_matcher.match_sec2_to_sec1_mfov(sec2_mfov_mesh_pts)
        logger.report_event("Features-Matching+PMCC layers: {} <- {} valid matches: {}, invalid_matches: {} {}".format(sec1.canonical_section_name, sec2.canonical_section_name, len(valid_matches2[0]), len(invalid_matches2[0]), FeaturesBlockMatcherDispatcher.sum_invalid_matches(invalid_matches2)), log_level=logging.INFO)
...
#         temp_ctr = 0
        for region1_key, sec1_region_mesh_pts in sec1_per_region_mesh_pts.items():
            ...
            res_pool = pool.apply_async(FeaturesBlockMatcherDispatcher._perform_matching, (region1_key, sec1, sec2, sec1_to_sec2_mfovs_transforms[region1_key[0]], sec1_cache[FeaturesBlockMatcherDispatcher.BLOCK_FEATURES_KEY], sec2_cache[FeaturesBlockMatcherDispatcher.BLOCK_FEATURES_KEY], sec1_region_mesh_pts, sec2_mesh_pts_cur_sec1_region, self._debug_dir, self._kwargs))
...
# #                 sec1_per_region_mesh_pts[tuple(sec1_pt_mfov_tile_idx)].append(sec1_pt)
# # 
# #         # Group the mesh pts of sec2 by the tile on sec1 which they should end up on (mfov1,tile1 that after applying its transformation is closest to that point)
# #         # Transform sec1 tiles centers to their estimated location on sec2
# #         sec1_tiles_centers_per_mfov = defaultdict(list)
# #         for sec1_tile_center, sec1_tiles_mfov_tile_idx in zip(sec1_tiles_centers, sec1_tiles_mfov_tile_idxs):
# #             sec1_tiles_centers_per_mfov[sec1_tiles_mfov_tile_idx[0]].append(sec1_tile_center)
...
# #         sec2_mesh_pts_mfov_tile_idxs = sec2_tiles_mfov_tile_idxs[sec2_tiles_centers_kdtree.query(sec2_mesh_pts)[1]]
# #         sec2_per_region1_mesh_pts = defaultdict(list)
# #         sec2_per_region1_sec2_mfov_tile_idxs = defaultdict(list)
# #         for sec2_pt, (sec2_pt_mfov_idx, sec2_pt_tile_idx), sec1_tile_center_idx in zip(sec2_mesh_pts, sec2_mesh_pts_mfov_tile_idxs, sec2_mesh_pts_sec1_closest_tile_idxs):
# #             sec2_tile = sec2.get_mfov(sec2_pt_mfov_idx).get_tile(sec2_pt_tile_idx)
# #             if BlockMatcherPMCCDispatcher._is_point_in_img(sec2_tile.bbox, sec2_pt):
# #                 sec2_per_region1_mesh_pts[tuple(sec1_tiles_mfov_tile_idxs[sec1_tile_center_idx])].append(sec2_pt)
# #                 sec2_per_region1_sec2_mfov_tile_idxs[tuple(sec1_tiles_mfov_tile_idxs[sec1_tile_center_idx])].append(tuple(sec2_pt_mfov_idx, sec2_pt_tile_idx))
# # 
# # 
# # 
# #         # Apply the matching procedure using multiple process (wokring on each mfov1,tile1 region)
# #         for sec1_tiles_mfov_tile_idx in sec1_tiles_mfov_tile_idxs:
# #             sec1_region1_mesh_pts = sec1_per_region_mesh_pts[tuple(sec1_pt_mfov_tile_idx)]
# #             sec2_region1_mesh_pts = sec2_per_region_mesh_pts[tuple(sec1_pt_mfov_tile_idx)]
# #             sec2_region1_mesh_pts_corresponding_mfov_tile_idxs = sec2_per_region1_sec2_mfov_tile_idxs[tuple(sec1_pt_mfov_tile_idx)]
# #             sec1_tile = sec1.get_mfov(sec1_pt_mfov_tile_idx[0]).get_tile(sec1_pt_mfov_tile_idx[1])
# #             sec2_tiles = {
# #                             sec2_mfov_tile_idx : sec2.get_mfov(sec2_mfov_tile_idx[0]).get_tile(sec2_mfov_tile_idx[1]) 
# #                                 for sec2_mfov_tile_idx in sec2_region1_mesh_pts_corresponding_mfov_tile_idxs
# #                          }
# #             sec1_to_sec2_mfov_transfom = sec1_to_sec2_mfovs_transforms[sec1_pt_mfov_tile_idx[0]]
# 
# 
# 
# 
# 
#         # Compute sec1_mesh_pts after transforming to sec2
#         sec1_tiles_centers = [
#                               [(t.bbox[0] + t.bbox[1])/2, (t.bbox[2] + t.bbox[3])/2]
#                              for t in sec1.tiles()]
#         sec1_tiles_centers_kdtree = KDTree(sec1_tiles_centers)
#         sec1_tiles_mfov_tile_idxs = np.array([[t.mfov_index, t.tile_index] for t in sec1.tiles()])
#         sec1_tiles_centers_on_sec2 = [sec1_to_sec2_mfovs_transforms[t_mfov].apply(sec1_tile_center) for sec1_tile_center, (t_mfov, _) in zip(sec1_tiles_centers, sec1_tiles_mfov_tile_idxs)]
#         sec1_tiles_centers_on_sec2_kdtree = KDTree(sec1_tiles_centers_on_sec2)
# 
#         # TODO - split the work in a smart way between the processes
#         sec1_mesh_pts_mfov_tile_idxs = sec1_tiles_mfov_tile_idxs[sec1_tiles_centers_kdtree.query(sec1_mesh_pts)[1]]
#         sec1_relevant_mesh_pts = []
#         sec1_relevant_mesh_pts_on_sec2 = []
#         for sec1_pt, sec1_pt_mfov_tile_idx in zip(sec1_mesh_pts, sec1_mesh_pts_mfov_tile_idxs):
#             sec1_tile = sec1.get_mfov(sec1_pt_mfov_tile_idx[0]).get_tile(sec1_pt_mfov_tile_idx[1])
#             if FeaturesBlockMatcherDispatcher._is_point_in_img(sec1_tile.bbox, sec1_pt):
#                 sec1_relevant_mesh_pts.append(sec1_pt)
#                 sec1_pt_on_sec2 = sec1_to_sec2_mfovs_transforms[sec1_tile.mfov_index].apply(sec1_pt)
#                 sec1_relevant_mesh_pts_on_sec2.append(sec1_pt_on_sec2)
# 
#         # Perform a per mesh_pt translation only matching (bidirectional)
#         for sec1_pt, sec1_pt_on_sec2 in zip(sec1_relevant_mesh_pts, sec1_relevant_mesh_pts_on_sec2):
#             rect_res = sec1_on_sec2_features_rtree.search([sec1_pt[0] - self._template_side, sec1_pt[0] + self._template_side,
#                                                            sec1_pt[1] - self._template_side, sec1_pt[1] + self._template_side])
#             sec1_features = [ for k, feature_idx in rect_res]
#             
# 
#         # Apply inverse transform to the matched points in sec1
# 
# 
# 
# 
