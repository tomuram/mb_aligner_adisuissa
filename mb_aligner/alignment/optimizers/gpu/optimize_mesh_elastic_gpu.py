...
class ElasticMeshOptimizerGPU(object):
    ...
    def _init_kernels(self):
        ...
        with open(os.path.join(dir_path, 'mesh_derivs_elatic_gpu.cu'), 'rt') as in_f:
            optimize_cu = in_f.read()

        mod_optimize_cu = SourceModule(optimize_cu)

        self._crosslink_mesh_derivs_func = mod_optimize_cu.get_function("crosslink_mesh_derivs")
        self._crosslink_mesh_derivs_func.prepare("PPPPPPiPPffP")
        self._internal_mesh_derivs_func = mod_optimize_cu.get_function("internal_mesh_derivs")
        self._internal_mesh_derivs_func.prepare("PPPiPffP")
        self._area_mesh_derivs_func = mod_optimize_cu.get_function("area_mesh_derivs")
        self._area_mesh_derivs_func.prepare("PPPiPfP")
        self._update_mesh_and_momentum_grads_func = mod_optimize_cu.get_function("update_mesh_and_momentum_grads")
        self._update_mesh_and_momentum_grads_func.prepare("PiPPff")
...
#         del cost_arr
    ...
    def _backup_meshes_gpu(meshes_gpu, block_lo, block_hi, prev_meshes_copy_gpu=None):
        ...
        if prev_meshes_copy_gpu is None:
            ...
            for sec_idx in range(block_lo, block_hi):
                gpuarray._memcpy_discontig(prev_meshes_copy_gpu[sec_idx], meshes_gpu[sec_idx])
    ...
    def _grad_descent_optimize(self, layout, block_lo, block_hi, meshes, links, norm_weights, structural_meshes):
        ...
        # Load data that's needed to the gpu (and allocate temp arrays)
        max_cost_array_len = 0
        for sec_idx in range(block_lo, block_hi):
            # make sure we got the gpu arrays for all the relevant meshes, links, structural meshes
            if sec_idx not in self._gpu_arrays["meshes"]:
                self._gpu_arrays["meshes"][sec_idx] = gpuarray.to_gpu(meshes[sec_idx].pts.astype(np.float32))
            max_cost_array_len = max(max_cost_array_len, self._gpu_arrays["meshes"][sec_idx].shape[0])
        ...
        old_meshes_gpu = self._backup_meshes_gpu(self._gpu_arrays["meshes"], block_lo, block_hi)
        cost_array_gpu = gpuarray.empty((max_cost_array_len,), np.float32, order='C')
        ...
        for iter in range(self._max_iterations):
            ...
            for arr in gradients_gpu.values():
                arr.fill(0.0)
...
#                                                               self._cross_slice_weight * norm_weights[sec1_idx, sec2_idx], self._cross_slice_winsor)
            ...
            if cost < prev_cost and not np.isinf(cost):
                ...
                #     gradients_with_momentum[sec_idx] = gradients[sec_idx] + momentum * gradients_with_momentum[sec_idx]
                #old_pts = {sec_idx: m.pts.copy() for sec_idx, m in meshes.items() if sec_idx in range(block_lo, block_hi)}
                old_meshes_gpu = self._backup_meshes_gpu(self._gpu_arrays["meshes"], block_lo, block_hi, old_meshes_gpu)
                # for sec_idx in range(block_lo, block_hi):
                #     meshes[sec_idx].pts -= stepsize * gradients_with_momentum[sec_idx]
                self._update_meshes_gpu(self._gpu_arrays["meshes"], stepsize, momentum, gradients_gpu, gradients_with_momentum_gpu, block_lo, block_hi)
                # if iter % 500 == 0:
                ...
                for sec_idx in range(block_lo, block_hi):
                    ...
                    gpuarray._memcpy_discontig(self._gpu_arrays["meshes"][sec_idx], old_meshes_gpu[sec_idx])
                stepsize *= 0.5
...
#                         #plot_points(meshes[active_ts].pts, os.path.join(DEBUG_DIR, "post_iter{}_{}.png".format(str(iter).zfill(5), os.path.basename(active_ts).replace(' ', '_'))))
    ...
    def _deallcate_gpu_arrays(self, min_sec_idx, max_sec_idx):
        for sec_idx in range(min_sec_idx, max_sec_idx):
            if sec_idx in self._gpu_arrays["meshes"]:
                del self._gpu_arrays["meshes"][sec_idx]
            if sec_idx in self._gpu_arrays["structural_meshes"]:
                del self._gpu_arrays["structural_meshes"][sec_idx]
