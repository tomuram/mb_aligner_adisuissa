...
class SectionMeshEdgesFilter(object):
    ...
    def __init__(self, mesh_spacing, **kwargs):
    ...
    def filter_edges_and_simplices(self, section):
        

        hex_grid = utils.generate_hexagonal_grid(section.bbox, self._mesh_spacing)

        mesh_edges_filter = MeshEdgesFilter(Delaunay(hex_grid))
        ...
        filtered_edges_indices, filtered_simplices = mesh_edges_filter.filter_by_wrinkles(all_image_contours)

        #visualize_filtered_mesh(mesh_tri, filtered_edges_indices, filtered_simplices, img)
        return filtered_edges_indices, filtered_simplices
